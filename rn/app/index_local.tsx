...
import { LocalAIConnection } from "./utils/localAI";
...
export default function App() {
  ...
  const [localAI] = useState(
    () => new LocalAIConnection()
  );

  async function startSession() {
    try {
      await localAI.startSession();
      setDataChannel(localAI.dataChannel);
    } catch (error) {
  ...
  function sendTextMessage(message: string) {
    ...
    localAI.sendClientEvent(messageEvent);
    localAI.sendClientEvent({ type: "response.create" });
  }
